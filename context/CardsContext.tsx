...
const CardsContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (!isLoaded) return;
    AsyncStorage.setItem(ACTIVE_CARDS, JSON.stringify(cards)).catch(
      console.error,
    );
  }, [cards, isLoaded]);
  ...
  useEffect(() => {
    if (!isLoaded) return;
    AsyncStorage.setItem(ARCHIVED_CARDS, JSON.stringify(archivedCards)).catch(
      console.error,
    );
  ...
  const addCard = (newCard: Card) => {
    if (
      cards.some(
        (card) => card.word.toLowerCase() === newCard.word.toLowerCase(),
      )
    )
      return "duplicate";
    if (
    ...
    setCards((prevCards) =>
      prevCards.map((card) => {
        if (card.word === word) {
          ...
          console.log(
            `[DEBUG] Marked as read: "${word}" → readCount: ${updated.readCount}`,
          );
          return updated;
        }
        return card;
